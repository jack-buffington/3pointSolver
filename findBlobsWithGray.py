...
def findGrayBlob(theImage, xPrior, yPrior):
	# Returns a single blob that is in a region around the prior location.
	# theImage is the saturation channel of the video.  Landmarks show up as lighter than the ceiling.  

	# Some constants that are used in this code:
	searchRegion = 20 # Widen this if there is more active movement in the input video.  

	theBlob = blob()	
	theBlob.xMin = theImage.shape[1]
	theBlob.yMin = theImage.shape[0]
	...
	for X in range(int(xPrior - searchRegion), int(xPrior + searchRegion)):
		if X >= 0 and X < theImage.shape[1]:
			for Y in range(int(yPrior - searchRegion), int(yPrior + searchRegion)):
				if Y >= 0 and Y < theImage.shape[0]:
					if theImage[Y,X] < histogramMin:
						histogramMin = theImage[Y,X]
					if theImage[Y,X] > histogramMax:
						histogramMax = theImage[Y,X]

	


	# Now that the maximum and minimum have been found, use a value roughly 50% of the way between them as the threshold
	ceilingThreshold = ((histogramMax - histogramMin) * 0.6) + histogramMin
	...
	for X in range(int(xPrior - searchRegion), int(xPrior + searchRegion)):
		if X >= 0 and X < theImage.shape[1]:
			for Y in range(int(yPrior - searchRegion), int(yPrior + searchRegion)):
				if Y >= 0 and Y < theImage.shape[0]:
					if theImage[Y,X] > ceilingThreshold:
						# Add this pixel to theBlob.
						if X < theBlob.xMin:
							theBlob.xMin = X 
						if X > theBlob.xMax: 
							theBlob.xMax = X 
						...
						theBlob.xWeight += X 
						theBlob.yWeight += Y 
						theBlob.pixelCount += 1 
	

	# Now that the blob has been found, compute its centroid
	theBlob.centroidX = theBlob.xWeight / theBlob.pixelCount
	theBlob.centroidY = theBlob.yWeight / theBlob.pixelCount
...
#while(vid.isOpened() and frameCount < 20):
while(vid.isOpened()):
	...
	try:
		...
		if ret == True:
			...
			if firstFrame == True:
				...
				#holidayGreen = ((v < 170) *out2).astype(np.uint8)   
				...
				blueBlob = neonBlueBlobs[biggestBlueBlob]
				greenBlob = holidayGreenBlobs[biggestGreenBlob]
				orangeBlob = orangeBlobs[biggestOrangeBlob]


				# blueBlob.centroidX = neonBlueBlobs[biggestBlueBlob].centroidX
				# blueBlob.centroidY = neonBlueBlobs[biggestBlueBlob].centroidY

				# greenBlob.centroidX = holidayGreenBlobs[biggestGreenBlob].centroidX
				# greenBlob.centroidY = holidayGreenBlobs[biggestGreenBlob].centroidY

				# orangeBlob.centroidX = orangeBlobs[biggestOrangeBlob].centroidX
				# orangeBlob.centroidY = orangeBlobs[biggestOrangeBlob].centroidY


				print(f"After frame 0: blue centroid: {blueBlob.centroidX}, {blueBlob.centroidY}")
			...
			else:  # it isn't the first frame so use the saturation channel only to determine where the landmarks are.

				#print(f"s.shape(): {s.shape}  s.shape[0]: {s.shape[0]}")
				#print(f"blue centroid: {blueBlob.centroidX}, {blueBlob.centroidY}")

				#print(f"blue centroid before call: {blueBlob.centroidX}, {blueBlob.centroidY}")
				blueBlob = findGrayBlob(s, blueBlob.centroidX, blueBlob.centroidY) 

				#print(f"blue centroid after call: {blueBlob.centroidX}, {blueBlob.centroidY}")
				greenBlob = findGrayBlob(s, greenBlob.centroidX, greenBlob.centroidY) 
				orangeBlob = findGrayBlob(s, orangeBlob.centroidX, orangeBlob.centroidY) 

				# Draw a rectange around each max
				# cv.rectangle(frame,(redBlobs[biggestRedBlob].xMin, redBlobs[biggestRedBlob].yMin),(redBlobs[biggestRedBlob].xMax, redBlobs[biggestRedBlob].yMax),(0,0,255),3)
				cv.rectangle(frame,(blueBlob.xMin, blueBlob.yMin),(blueBlob.xMax, blueBlob.yMax),(255,0,0),3)
				cv.rectangle(frame,(orangeBlob.xMin, orangeBlob.yMin),(orangeBlob.xMax, orangeBlob.yMax),(0,128,255),3)
				cv.rectangle(frame,(greenBlob.xMin, greenBlob.yMin),(greenBlob.xMax, greenBlob.yMax),(0,255,0),3)

				file1.write(f"{blueBlob.centroidX}, {blueBlob.centroidY}, {orangeBlob.centroidX}, {orangeBlob.centroidY}, {greenBlob.centroidX}, {greenBlob.centroidY}, 0, 0, 0, 0, 1\n")
		...
		print(f"Error location : {errorLocation}\n" )
